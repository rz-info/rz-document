...
<el-row style="margin-top:30px" class="table_his">
	<commonTable :tableMap="tableMap" :tableData="tableData" @editEject="editEject" :tableConfig="tableConfig">
		<template v-slot:btn="{ rowData, template, scope }">
			<template v-for="(item,key) in template">
				<div v-if="key == 'img'" :key="key">
					...
					<img style="margin-left:20px;" src="@/views/classes/icon/sanjiao.png" @click.stop="selectEditHistorical(scope.$index,rowData)" alt="">
...
// 					 })
				this.LedgerHistory=[{updatedAt:"000000",modifyContent:"内容999999999999"},{updatedAt:"000000",modifyContent:"内容999999999999"}]
			});
